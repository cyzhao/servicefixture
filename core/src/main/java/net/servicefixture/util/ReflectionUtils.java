...
public final class ReflectionUtils {
	...
	public static Object newInstance(Class clazz) {
		try {
			...
			throw new ServiceFixtureException(
					"Unable to create the new instance of class:"
							+ clazz.getName(), e);
		}
	}
	...
	public static Class newClass(String clazzName) {
		try {
			...
			throw new ServiceFixtureException("Class:" + clazzName
					+ " can not be found.");
		}
	}
	...
	public static Class getAttributeType(Object object, String attributeName) {
		try {
			...
			throw new ServiceFixtureException("Unable to find the attribute:"
					+ attributeName + " in class: "
					+ object.getClass().getName());
		}
	}

	public static void setObjectAttribute(Object parent, String attributeName,
			Object source) {
		try {
			if (source == null) {
				Class attributeType = getAttributeType(parent, attributeName);
				if (attributeType.isPrimitive()
						|| isPrimitiveWrapper(attributeType)) {
			...
			throw new ServiceFixtureException("Unable to set the attribute:" + attributeName + " on object:" + parent + " as:" + source, e);		
		}
	}
	...
	public static void attacheObjectToParent(Object parent,
			String attributeName, Object source) {
	...
	public static Map<String, Class> getAttributes(Class type) {
		...
		for (int i = 0; i < methods.length; i++) {
			if (methods[i].getName().startsWith("get")
				...
				String attributeName = StringUtils.uncapitalize(methods[i]
						.getName().substring(3));
				Class returnType = methods[i].getReturnType();
				if (CollectionBuilderFactory.isCollectionType(returnType)) {
					attributeName = attributeName + "[0]"
							+ getExtraMultiDimensionalArraySuffix(returnType);
					returnType = CollectionBuilderFactory
	...
	public static String getDefaultValue(Class type) {
		if (isEnumarationPatternClass(type)) {
			...
			for (int i = 0; i < fields.length; i++) {
				if (type.equals(fields[i].getType())
	...
	public static Class[] findSubClasses(Class baseClass) {
		String packagePath = "/"
				+ baseClass.getPackage().getName().replace('.', '/');
			...
			if ( connection instanceof JarURLConnection ) {
				...
				while (e.hasMoreElements()) {
					...
					if (entryName.endsWith(".class")) {
						String clazzName = entryName.substring(0, entryName
								.length() - 6);
		...
		return derivedClasses
				.toArray(new Class[derivedClasses.size()]);
	}
	...
	private static boolean isConcreteSubclass(Class<?> baseClass, Class<?> clazz) {
		return baseClass.isAssignableFrom(clazz) && !baseClass.equals(clazz)
				&& !clazz.isInterface()
